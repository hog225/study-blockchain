App = {
  ...
  bindEvents: function () {
    ...
    $(document).on('click', '#show-hashval', App.handleTestHash);
      ...
      .then(function (result) {
        ...
        if (result) {
          if (parseInt(result.receipt.status) == 1) {
            ...
            App.contracts.blindAuction.deployed().then(function(latestInstance) {
  ...
  handleTestHash: function () {
    var numberVal = parseInt($("#bid-number-value").val());
    App.contracts.blindAuction.deployed().then(function (instance) {
      console.log("number " + numberVal)
      return instance.showBlindedBid(numberVal);
    }).then(function (result) {
      console.log(result.logs[0].args['blindedBid'])
      jQuery('#hashed_val').text(result.logs[0].args['blindedBid']);
